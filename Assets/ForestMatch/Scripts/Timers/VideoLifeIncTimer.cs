...
namespace Mkey
{
    public class VideoLifeIncTimer : MonoBehaviour
    {
        private GlobalTimer gTimer;
        private string videoIncTimerName = "VideoIncLife";
        [Tooltip("Time span to life increase, minutes")]
        [SerializeField]
        private int videoIncTime = 60;
        ...
        public static VideoLifeIncTimer Instance;
        ...
        void Start()
        {
            ...
            VPurch.VideoRewardCompleteLife += ChangeVideoHandler;

            if ((VPurch?.countVideoLifes < incIfLessThan) && !IsWork)
            {
        ...
        {
            if (VPurch.countVideoLifes < incIfLessThan)
            {
                VPurch.VideoCountLifeAdd(1);
            }
...
#if UNITY_EDITOR
    [CustomEditor(typeof(VideoLifeIncTimer))]
    public class VideoLifeTimerEditor : Editor
    {
