...
namespace Mkey
{
    public class HeaderGUIController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                Refresh();
                //MPlayer.ChangeScoreEvent += RefreshCoinStrip;
        ...
        private void OnDestroy()
        {
            ...
            MPlayer.ChangeStarsEvent -= RefreshStars;
            MPlayer.ChangeStarsEvent -= GetStarsSound;
            //if (CoinsCount) SimpleTween.Cancel(CoinsCount.gameObject, false);
        ...
        public void Refresh()
        {
          ...
          //  RefreshCoinStrip(Mathf.RoundToInt(MPlayer.LevelScore / 100), MPlayer.AverageScore);
        }
        ...
        #region coin
        internal void RefreshCoinStrip(int coin, int averageCoin)
        {
            // refresh coin text
            if (CoinsCount)
            {
                int newCount = coin;
                SimpleTween.Cancel(CoinsCount.gameObject, false);
                SimpleTween.Value(CoinsCount.gameObject, oldCount, newCount, 0.5f).SetOnUpdate((float val) =>
                {
                    oldCount = (int)val;
                    ...
                    CoinsCount.text = oldCount.ToString();
                });
            }

            //float amount = (averageCoin > 0) ? (float)coin / (float)(averageCoin * 2.0f) : 0;
            //if (CoinsStrip)
            //    SimpleTween.Value(ScoreStrip.gameObject, ScoreStrip.fillAmount, amount, 0.3f).SetOnUpdate((float val) =>
            //    {
            //        CoinsStrip.fillAmount = val;
