...
namespace Mkey
{
    public class MatchPlayer : MonoBehaviour
    {
        ...
        private MapController MapContr { get { return MapController.Instance; } }
        ...
        [SerializeField]
        private int priceCoinsContinue = 50;

        [Tooltip("Default coins at start")]
        [SerializeField]
        private int koeffCoinsscore = 400;

        [Tooltip("Default coins at start")]
        [SerializeField]
        private int defCoinsCount = 500;

        [Tooltip("Default fortunes at start")]
        [SerializeField]
        private int defFortuneCount = 0;
     ...
     //   [SerializeField]
        ...
        private string saveFortunaKey = savePrefix + "fortuna"; // current fortunes
        ...
        private string saveRateMoved = savePrefix + "rateMoved"; // saved flag for facebook coins (only once)

        private string SaveLevelStarsKey { get { int l = CurrentLevel + 1;  return l.ToString() + savePrefix + "levelstars"; } }  // level stars
        private string SaveLevelScoresKey { get { int l = CurrentLevel + 1; return l.ToString() + savePrefix + "levelscores"; } } // level score
        ...
        public Action <int> ChangeFortuneEvent;
        ...
        public int priceCoinsCont { get { return priceCoinsContinue; } }
        public int koeffCoinsScore { get { return koeffCoinsscore; } }
        ...
        private void Start()
        {
         ...
         //   PlayerPrefs.SetInt(saveRateMoved, 0);
        ...
        private void SetStarsByScore()
        {
            ...
            if      (LevelScore > (AverageScore * 2f * starPos_3)) stars = 3;
            else if (LevelScore > (AverageScore * 2f * starPos_2)) stars = 2;
            else if (LevelScore > (AverageScore * 2f * starPos_1)) stars = 1;
		    SetStarsCount(stars);
        }
        #endregion stars
        ...
        public void AddFortunes(int count)
        {
            SetFortuneCount(Fortune + count);
        }

        public void SetDefaultFortune()
        {
            SetFortuneCount(defFortuneCount);
        }

        public void SetFortuneCount(int fortuns)
        {
            fortuns = Mathf.Max(0, fortuns);
            bool changed = (Diamonds != fortuns);
            Fortune = fortuns;
            if (SaveData && changed)
            {
                string key = saveFortunaKey;
                PlayerPrefs.SetInt(key, Fortune);
            }
            if (changed) ChangeFortuneEvent?.Invoke(Fortune);
        }
        ...
        /// </summary>
        /// 
        public void RateMove()
        {
            if (!PlayerPrefs.HasKey(saveRateMoved) || PlayerPrefs.GetInt(saveRateMoved) == 0)
            {
                PlayerPrefs.SetInt(saveRateMoved, 1);
            }
        }

        public int RateGetMove()
        {
            return PlayerPrefs.GetInt(saveRateMoved);
        }
        /// 
        ...
        private void LoadFortune()
        {
            ...
            {
                string key = saveFortunaKey;
                if (PlayerPrefs.HasKey(key)) SetFortuneCount(PlayerPrefs.GetInt(key));
                else SetFortuneCount(defFortuneCount);
            }
            else
            {
                SetFortuneCount(defFortuneCount);
        ...
        {
            if (CurrentLevel + 1 <= MBoard.GCSet.LevelCount)
            {
            ...
            // AddCoins(StarCount);
            AddCoins(Mathf.RoundToInt(LevelScore / koeffCoinsscore));
            ...
            if (SaveData)
            {
                ...
                if (oldStarCount < StarCount)
                {
                    ...
                    //  MapContr.SetButtonActive(CurrentLevel+2, true, true);

                    if (StarCount >= 3)
                    {
                        MGui.ShowFindChest(null);
                        AddFortunes(1);
                    }
                }
        ...
        public int GetBooster(int id)
        {
            Booster b = BoostHolder.GetBoosterById(id);
            if (b != null)
                return b.Count;
            return 0;
        }

        public void AddBooster(int id, int count)
        {
            Booster b = BoostHolder.GetBoosterById(id);
            if (b != null)
                b.AddCount(count);
        ...
        public void SetDefaultData()
        {
            ...
            PlayerPrefs.SetInt(saveStatPurKey, 0);
            SetDefaultLife();
            SetDefaultHardMode();
			SetDefaultFullName();
			EndInfiniteLife();
