...
namespace Mkey
{
    public class Booster
    {
        ...
        {
            CreateSceneObject(SortingOrder.Booster, null, footerBooster.instantiatePosition ? footerBooster.instantiatePosition : footerBooster.GetComponent<RectTransform>());
            ActiveBooster = this;
        ...
        public GameObject DuplicateGameobjet()
        {
            GameObject g =  UnityEngine.Object.Instantiate(bData.prefab).gameObject;
            Vector3 wPos = Vector3.zero;

            if (SceneObject != null)
            {
                wPos = SceneObject.transform.position;
                g.transform.position = wPos;
                g.transform.parent = SceneObject.transform;
                SpriteRenderer sr = SceneObject.GetComponent<SpriteRenderer>();
                SpriteRenderer sr1 = g.GetOrAddComponent<SpriteRenderer>();
                sr1.sortingOrder = sr.sortingOrder;
            }
            return g;
        }

        public void CreateSceneObject(int sortingOrder, Transform parent, RectTransform guiObject)
        {
