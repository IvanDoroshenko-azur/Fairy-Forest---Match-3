...
namespace Mkey
{
    public class MatchGrid
    {
      ...
      //      Debug.Log("rows: " + Rows.Count +  " ;columns count: " + columns.Count);
        ...
        public MatchGroupsHelper GetMatches4()
        {
            MatchGroupsHelper mgh = new MatchGroupsHelper(this);
            mgh.CreateGroups(3, false);
            return mgh;
        }
        ...
        internal List<GridCell> GetFreeCells()
        {
            ...
            for (int j = 0; j < Cells[0].GColumn.Length; j++) // Количество рядов
            {
                for (int k = 0; k < Cells[0].GRow.Length; k++) // Беребираем каждый ряд
                {
                    if (Cells[count].spawner)
                    {
                        if (k == 0)
                        {
                        ...
                        else
                        {
                            if (!Cells[count].Blocked && !Cells[count].IsDisabled && ((Cells[count].Overlay != null) ? Cells[count].Overlay.CanSkip : true)) // Если свободен?
                            {
                                ...
                                if (check[count - 1] == -1) //  Если сосед слева из черного, то переводим его в белый
                                {
                                    check[count - 1] = 1;
                                    if(k-2 >= 0)
                                    if (check[count - 2] == -1) //  Если сосед слева из черного, то переводим его в белый
                                    {
                                        check[count - 2] = 1;
                                        if (k - 3 >= 0)
                                        if (check[count - 3] == -1) //  Если сосед слева из черного, то переводим его в белый
                                        {
                                            check[count - 3] = 1;
                                            if (k - 3 >= 0)
                                            if (check[count - 4] == -1) //  Если сосед слева из черного, то переводим его в белый
                                            {
                                                check[count - 4] = 1;
                                            }
                                        }
                    ...
                    else
                    {
                        if (!Cells[count].Blocked && !Cells[count].IsDisabled && ((Cells[count].Overlay != null) ? Cells[count].Overlay.CanSkip : true)) // Если свободен?
                        {
                            ...
                            else if (check[count - 1] == 1) //  Если сосед слева из белого, то переводим себя в белый
                            {
                                check[count] = 1;
                            }
                            else
        ...
        internal void FillGrid(bool noMatches)
        {
            ...
            for (int i = 0; i < Cells.Count; i++)
            {/////////////////////////////////////
                if (!Cells[i].Blocked && !Cells[i].IsDisabled && !Cells[i].DynamicObject && (Cells[i].Overlay ? Cells[i].Overlay.CanSkip : true))
                {
                    int id = (goSet.GetMainRandomObjects(1))[0].ID;//  Debug.Log("fill " + Cells[i] + " id: " + id);
                    Cells[i].SetObject(id);
                }
            }
            if (noMatches)
            {
        ...
        internal void RemoveMatches()
        {
            ...
            for (int i = 0; i < vertSize; i++)
            {
                for (int j = 0; j < horSize; j++)
                {
                    ...
                    if (mod_list.Count > 0) Rows[i][j].GetComponent<GridCell>().SetMatchObject((goSet.GetMainRandomObjects(1, mod_list))[0]);
                }
            }
#if UNITY_EDITOR
            // double test
