...
namespace Mkey
{
    ...
    {
        #region properties
        public DynamicMatchBombObjectData OData { get; private set; }
        ...
        #region create
        internal virtual void SetData(DynamicMatchBombObjectData oData)
        {
            SRenderer = GetComponent<SpriteRenderer>();
          //  if (SRenderer) SRenderer.sprite = (oData != null) ? oData.ObjectImage : null;
            OData = oData;
#if UNITY_EDITOR
            gameObject.name = (oData != null) ? "DynamicMatchBomb: " + GetID() + "(" + SRenderer.sprite.name + ")" : "none";
#endif
            SetToFront(false);
        }
        ...
        /// <returns></returns>
        public static DynamicMatchBombObject Create(MatchObject parent, DynamicMatchBombObjectData oData, bool addCollider, bool isTrigger, Action<int> TargetCollectEvent)
        {
            if (!parent || oData == null) return null;

            Sprite bombSprite = oData.ObjectImage;
            switch (oData.bombType)
            {
                case BombDir.Vertical:
                    bombSprite = parent.OData.VerticalGift;
                    break;
                case BombDir.Horizontal:
                    bombSprite = parent.OData.HorizontalGift;
                    break;
                case BombDir.Radial:
                    bombSprite = parent.OData.CrossGift;
                    break;
            }
          //  Debug.Log(bombSprite);
            SpriteRenderer sR = Creator.CreateSprite(parent.transform, bombSprite, parent.transform.position);
            GameObject gO = sR.gameObject;
            DynamicMatchBombObject gridObject = gO.AddComponent<DynamicMatchBombObject>();
