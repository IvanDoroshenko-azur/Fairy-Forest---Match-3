        ...
        private void CreateMap(MatchGrid grid)
        {
          ...
          //  UnityEngine.Debug.Log("Create new map");
            ...
            {///////////////
                ...
                pfc.available = (!item.IsDisabled && !item.Blocked && (item.Overlay ? item.Overlay.CanSkip : true));
                item.pfCell = pfc;
                pfCells.Add(pfc);
            }
            ...
            if (gCount == pfCount && grid.Cells[0] == pfCells[0].gCell && grid.Cells[gCount-1] == pfCells[pfCount-1].gCell)
            {
               // UnityEngine.Debug.Log("Refresh map");
                ...
                {/////////////////////////
                    ...
                    item.available = (!item.gCell.IsDisabled && !item.gCell.Blocked && (item.gCell.Overlay ? item.gCell.Overlay.CanSkip : true));
                }
            }
            else
            {
