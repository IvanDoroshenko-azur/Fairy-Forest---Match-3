...
namespace Mkey
{
    public enum RealShopType { None, Coins, Life, Pack };
 
    // Deriving the Purchaser class from IStoreListener enables it to receive messages from Unity Purchasing.
    public class Purchaser : MonoBehaviour, IStoreListener
    {
        [Header("Consumables Pack: ", order = 1)]
        public ShopPackThingDataReal[] consumablePack;
        ...
        public string appKey = "com.whitesarcazm.forestmatch3"; 
        public string googleKey = "com.whitesarcazm.forestmatch3";
        private enum Boosters:int{ BombColor = 0, Bomb, Shuffle, Hammer};       
        ...
        public void InitializePurchasing()
        {
            ...
            if (consumablePack != null && consumablePack.Length > 0)
            {
                for (int i = 0; i < consumablePack.Length; i++)
                {
                    if (consumablePack[i] != null && !string.IsNullOrEmpty(consumablePack[i].kProductID))
                    {
                        string prodID = consumablePack[i].kProductID;
                        ...
                        consumablePack[i].clickEvent.RemoveAllListeners();
                        consumablePack[i].clickEvent.AddListener(() => { BuyProductID(prodID); });
                    }
                }
            }
...
#region IStoreListener
        ...
        {
            ShopPackThingDataReal prodPack = GetProductPackById(args.purchasedProduct.definition.id);
            ShopThingDataReal prod = GetProductById(args.purchasedProduct.definition.id);


           if (prodPack != null)
            {
                Debug.Log(string.Format("ProcessPurchase: PASS. Product: '{0}'", prodPack.kProductID));
                if (prodPack.PurchaseEvent != null)
                {
                    AnalyticsEvent.Custom("Purchaise_", new Dictionary<string, object>
                        {
                         { "Level_", MatchPlayer.CurrentLevel  }, {" Packet_", prodPack.kProductID}
                        });

                    prodPack.PurchaseEvent.Invoke();
                }
                else
                {
                    Debug.Log("PurchasePackEvent failed");
                }

                GoodPurchaseMessage(prodPack.kProductID, prodPack.name.ToString());
            }
            else if (prod != null)
            {
                ...
                if (prod.PurchaseEvent != null)
                {
                    AnalyticsEvent.Custom("Purchaise_", new Dictionary<string, object>
                        {
                         { "Level_", MatchPlayer.CurrentLevel  }, {" Produkt_", prod.kProductID}
                        });
        ...
        public void BoosterAdd(int id, int count)
        {
            Booster b = MPlayer?.BoostHolder.GetBoosterById(300000 + id);
            if (b != null)
                b.AddCount(count);
        }

        public void BombsAdd(int count)
        {
            BoosterAdd((int)(Boosters.BombColor), count);
            BoosterAdd((int)(Boosters.Bomb), count);
            BoosterAdd((int)(Boosters.Shuffle), count);
            BoosterAdd((int)(Boosters.Hammer), count);
        }
        ...
        {
            if (MatchPlayer.Instance)
            {
                MatchPlayer.Instance.AddCoins(count);
            }
        }
        ...
        public void AddLife(int count)
        {
            if (MatchPlayer.Instance)
            {
                MatchPlayer.Instance.AddLifes(count);
            }
        }
        ...
        public void SetInfiniteLife(int count)
        {
            if (MatchPlayer.Instance)
            {
                MatchPlayer.Instance.StartInfiniteLife(count);
            }
        }

        private void GoodPurchaseMessage(string prodId, string prodName)
        {
            MobAds.PurchDoNotAds();

            if (MatchGUIController.Instance)
            {
                MatchGUIController.Instance.ShowMessage("Succesfull!!!", "Congratulations on your purchase", 3, null);
            }
        }
        ...
        private void FailedPurchaseMessage(string prodId, string prodName)
        {
            if (MatchGUIController.Instance)
            {
                MatchGUIController.Instance.ShowMessage("Sorry!", "Purchase failed.", 3, null);
            }
        }
        ...
        ShopPackThingDataReal GetProductPackById(string id)
        {
            if (consumablePack != null && consumablePack.Length > 0)
                for (int i = 0; i < consumablePack.Length; i++)
                {
                    if (consumablePack[i] != null)
                        if (String.Equals(id, consumablePack[i].kProductID, StringComparison.Ordinal))
                            return consumablePack[i];
                }
...
#region build 
        ...
        private void GoodPurchaseMessage(string prodId, string prodName)
        {
            if (MatchGUIController.Instance)
            {
                MatchGUIController.Instance.ShowMessage("Succesfull!!!", prodName + " purchased successfull.", 3, null);
            }
        }
        ...
        private void FailedPurchaseMessage(string prodId, string prodName)
        {
            if (MatchGUIController.Instance)
            {
                MatchGUIController.Instance.ShowMessage("Sorry!", prodName + " - purchase failed.", 3, null);
            }
        }
