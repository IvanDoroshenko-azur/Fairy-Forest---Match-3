...
namespace Mkey
{
    public class TouchManager : MonoBehaviour
    {
        ...
        {
            Source = source;
            if (source ? source.Match : false)
            {
                MatchObject tm = source.Match;
                Draggable = tm.gameObject;//source.Match;
                startDragPos = tm.transform.position;//source.transform.position;
                //Draggable = source.Match;
                //startDragPos = source.transform.position;
            }
            else if (source ? source.DynamicClickBomb: false)
            {
                DynamicClickBombObject dm = source.DynamicClickBomb;
                Draggable = dm.gameObject;// source.DynamicClickBomb;
                startDragPos = dm.transform.position;//source.transform.position;
            }
            else
            {
                Draggable = null;
